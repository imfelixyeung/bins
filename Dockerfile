...
HEALTHCHECK --interval=1s --timeout=30s --start-period=5s --retries=10 CMD wget -O/dev/null -q http://$HOSTNAME:3000/api/health || exit 1
...
HEALTHCHECK --interval=1s --timeout=30s --start-period=5s --retries=10 CMD wget -O/dev/null -q http://$HOSTNAME:3000/health || exit 1
